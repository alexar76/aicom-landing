...
import { isDemoMode } from "./lib/demoMode.mjs";
...
const APP_PAGE_CSP = [
  ...
  // public/index.html links its own public/fonts/fonts.css, so 'self' is enough for both the
  // stylesheet and the woff2 files. Google's origins are deliberately not named: a font
  // request that leaves this host should fail visibly rather than work.
...
async function handleApiGenerate(req, res) {
  ...
  const firewall = inspectGeneratePrompt(prompt);
  if (firewall.reject) {
    json(res, 400, {
      error: firewall.message,
  ...
  try {
    if (isDemoMode()) {
      const demoPath = join(ROOT, "docs", "examples", "saas-task-manager.html");
    ...
    const { html, stylePreset, agentToWebsite: agentMode } = await generateLanding({
      userPrompt: wrapUntrustedBrief(firewall.cleaned || prompt),
      styleId: style,
      uiLocale,
      rootDir: ROOT,
      agentToWebsite,
...
async function handleStatic(req, res) {
  ...
  if (pathname === "/api/config" && req.method === "GET") {
    ...
    json(res, 200, {
      ...
      public_ui: publicUi,
      llm_ready: isLlmConfigured() || isDemoMode(),
  ...
  if (pathname === "/" || pathname === "/index.html") {
    ...
    if (isPublicUi()) {
      ...
      html = html.replace(/>local uniquement</i, ">Live<");
      html = html.replace(/>仅本地</i, ">Live<");
    }
    res.writeHead(200, {
      ...BASE_SECURITY_HEADERS,
  ...
  // The self-hosted webfonts index.html links as fonts/fonts.css. Without this route every
  // request for them fell through to the 404 below, and under nosniff the page silently
  // rendered in fallback faces. One flat directory, names from a fixed alphabet: no traversal.
  const font = /^\/fonts\/([A-Za-z0-9._-]+\.(css|woff2))$/.exec(pathname);
  if (font && req.method === "GET" && !font[1].startsWith(".")) {
    try {
      const body = await readFile(join(PUBLIC_DIR, "fonts", font[1]));
      res.writeHead(200, {
        ...BASE_SECURITY_HEADERS,
        "content-type": font[2] === "css" ? "text/css; charset=utf-8" : "font/woff2",
        "cache-control": font[2] === "css" ? "public, max-age=3600" : "public, max-age=31536000, immutable",
      });
      res.end(body);
    } catch {
...
function main() {
  ...
  server.listen(port, host, () => {
    ...
    console.log(`UI locale: ${ui} (set AICOM_LANDING_UI_LOCALE or ?lang=ru|es|fr|zh)`);
    console.log(
      badge.enabled
