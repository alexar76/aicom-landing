 ...
 * @param {{ avoidAgentFab?: boolean }} [opts]
 * @returns {string}
 */
export function badgeHtmlSnippet(config, opts = {}) {
  if (!config.enabled) return "";
  const href = escapeHtmlAttr(config.url);
  const text = escapeHtmlText(config.label);
  // When the agent FAB sits bottom-right, keep the badge left of it and slightly lower.
  const position = opts.avoidAgentFab
    ? "bottom:0.625rem;right:5.85rem;"
    : "bottom:16px;right:16px;";
  return (
    `<a id="${BADGE_ID}" href="${href}" target="_blank" rel="noopener noreferrer" ` +
    `style="position:fixed;${position}z-index:999999;padding:8px 12px;` +
 ...
 * @param {{ avoidAgentFab?: boolean }} [opts]
 */
export function applyBadgeToHtml(html, config = getBadgeConfig(), opts = {}) {
  let out = stripBadgeFromHtml(html);
  if (!config.enabled) return out;
  const avoidAgentFab =
    opts.avoidAgentFab ?? /id=["']aicom-agent["']/i.test(out);
  const snippet = badgeHtmlSnippet(config, { avoidAgentFab });
