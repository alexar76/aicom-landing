...
export function strengthenHeroBackground(html) {
  ...
  if (heroOpen) {
    const tag = heroOpen[0];
    const patched =
      /style=/i.test(tag) && !/position\s*:\s*relative/i.test(tag)
        ? tag.replace(/style=(["'])/i, 'style=$1position:relative;overflow:hidden;')
        : /style=/i.test(tag)
          ? tag
          : tag.replace(/>$/, ' style="position:relative;overflow:hidden">');
    out = out.replace(tag, `${patched}\n${layers}`);
  }
